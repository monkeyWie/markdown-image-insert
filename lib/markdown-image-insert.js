...
export default {
  ...
  activate(state) {
    // this.markdownImageInsertView = new MarkdownImageInsertView(state.markdownImageInsertViewState);
    // this.modalPanel = atom.workspace.addModalPanel({
    //   item: this.markdownImageInsertView.getElement(),
    //   visible: false
    // });
  ...
  serialize() {
    // return {
    //   markdownImageInsertViewState: this.markdownImageInsertView.serialize()
    // };
  },
  ...
  paste() {
    ...
    if (!image.isEmpty()) {
      ...
      if (suffix == ".md" || suffix == ".markdown") {
        ...
        const imgPath = join(filePath, new Date().getTime() + ".png")
        fs.writeFileSync(imgPath, image.toPng ? image.toPng() : image.toPNG())
        textEditor.insertText("![](" + imgPath + ")");
        return
      }
    }
  }
